...
**8.3 副屏不承载管理 UI**：不复刻 Nexus 底部 dock（文件夹/媒体库）。682px 屏只放 widget 与内容；配置若将来需要 GUI，做主屏小窗（M4 后再议）
...
**M4** 背景动画引擎（含像素宠物类前景精灵，是否做见 §13-Q5）、AI 用量 widget、配置 GUI 评估、LED 灯效接管（**可选，可砍不影响主线**；需先逆向，参考 §2 的外部项目）。
