//! 守护 auxdeck-shell.exe：在 daemon 同目录查找，存在则拉起并监控，异常退出时
...
const SHELL_EXE_NAME: &str = "auxdeck-shell.exe";
...
/// 持续监控 auxdeck-shell.exe。若同目录下不存在该可执行文件，记录一次 warn 后
    ...
    loop {
        ...
        match Command::new(&shell_path).spawn() {
            Ok(mut child) => match child.wait().await {
                Ok(status) => info!("auxdeck-shell exited with {status}"),
                Err(err) => error!("failed to wait on auxdeck-shell process: {err}"),
            },
        ...
        warn!("restarting auxdeck-shell in {backoff:?}");
